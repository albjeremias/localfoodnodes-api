...
export default {
  ...
  /**
   * Get order count per node
   */
  countPerNode() {
    return db.query('SELECT data FROM statistics WHERE statistics.key = ?', ['order_count_per_node'])
    .then(results => {
      return {
        data: JSON.parse(results.data)
      };
  ...
  /**
   * Get order amount per node in euro
   */
  amountPerNode(currency) {
